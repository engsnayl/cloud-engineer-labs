# Solution Walkthrough — Lab 036: Container Logging
...
| `lab036-container-logging` | The name of the container |
...
| `lab036-container-logging` | Which container to reach into |
...
Edit `/opt/app.py` inside the container (using `vi` or `nano` via `docker exec -it lab036-container-logging vi /opt/app.py`). The fix is two lines:
...
```bash
docker restart lab036-container-logging
```
...
| `lab036-container-logging` | The container name |
