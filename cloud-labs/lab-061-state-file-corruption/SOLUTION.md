...
> ⚠️ This lab requires a real AWS account. Charges may apply — destroy all resources at the end.

### Before you start — clean state check

If you are re-running this lab, make sure there are no leftover files from a previous run:
...
If these files don't exist, that's fine — the commands will do nothing.
...
| `terraform apply` | Creates the real AWS resources and writes the state file |

Type `yes` when prompted. After apply you should have:
- An S3 bucket with tags `staging/ops` and versioning `Disabled`
- A security group `app-sg`
- A random ID suffix

This is your baseline. The state file now reflects what exists in AWS.

### Step 2: Simulate the console changes

You are now going to play the role of someone who went rogue in the AWS console. Run:
...
The script makes three real AWS API calls — exactly what the console does under the hood:
1. Changes the bucket tags to `production/engineering`
2. Enables bucket versioning
3. Deletes the security group

Terraform's state file still reflects the original apply. It has no idea any of this happened.
...
Read the output carefully. Your job starts here. The rest of this solution walks through what you're seeing and how to fix it.
...
1. *Why is Terraform planning to create it rather than erroring?* The AWS provider detected the SG is gone during the refresh phase and updated its understanding. It knows the resource no longer exists, so rather than trying to manage a dead ID it plans to create a fresh one.
...
| File/Directory | Why you delete it |
|---------------|------------------|
| `terraform.tfstate` | Stale state causes the next run to think resources already exist |
| `terraform.tfstate.backup` | Previous state backup — no longer needed after a clean destroy |
| `.terraform/` | Downloaded provider plugins — regenerated by `terraform init` |
| `.terraform.lock.hcl` | Provider version lock — regenerated by `terraform init` |
...
A directory created by `terraform init`. Contains the downloaded provider plugins (the AWS provider, the random provider, etc.) and internal Terraform metadata. Safe to delete — always regenerated by `init`.

### `.terraform.lock.hcl`

A lock file created by `terraform init`. Records the exact versions of providers that were selected, so every subsequent `init` uses the same versions. Similar in concept to `package-lock.json` in Node or `Gemfile.lock` in Ruby. Commit this to git in real projects — delete it for a clean lab reset.
...
AWS charges for running resources even when you're not using them. Always `terraform destroy` at the end of a lab, followed by the cleanup commands.
